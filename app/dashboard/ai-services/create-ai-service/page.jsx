...
export default function CreateAiServices() {
  ...
  //     const blogDataRaw = {
  //       // Additional fields from service form
  //       shortDescriptionHeading: formData.shortDescriptionHeading || "",
  //       aboutServiceHeading: formData.aboutServiceHeading || "",
  //       aboutServiceDescription: formData.aboutServiceDescription || "",
  ...
  //     // const uploadrawdata = new FormData();
  //     uploadFormData.append("blog_data_raw", JSON.stringify(blogDataRaw));
  ...
  const handleSubmit = async (formData) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Service creation failed");
      }
